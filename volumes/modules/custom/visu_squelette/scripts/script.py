...
# Table HTML pour stocker les clics
clicked_data = []

# Fonction de clic sur une image
def on_click(event):
    if event.xdata is None or event.ydata is None:
        return
    for (x_min, x_max, y_min, y_max, row) in image_positions:
        if x_min <= event.xdata <= x_max and y_min <= event.ydata <= y_max:
            clicked_data.append(row)
            generate_html_table()
            break

# Générer un fichier HTML avec les données des clics
def generate_html_table():
    html_content = """
    <html>
    <head><title>Tableau des Clics</title></head>
    <body>
    <table border="1">
        <tr>
            <th>Entité</th>
    ...
    """
    with open("clicked_data.html", "w") as f:
